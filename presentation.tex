...
\begin{frame}
Метод и работает в режиме реального времени, однако когда допущения описанного метода неверны, карта проходимости, построенная с его помощью, может содержать грубые ошибки: 
...
\begin{itemize}
  \item Первый метод основан на прямой модели, карта проходимости находится с помощью метода стохастического градиента.
...
\begin{itemize}
  \item $\psi_{free} = \psi_{free}(X_{free})$ - значение зависит от суммарной эффективной занятости $X_{free}$ внутри $\Omega_{free}$. Если $X_{free}$ больше некоторого порога, то значение штрафа достаточно велико, иначе - небольшое.
  
  \item $\psi_{occ} = \psi_{occ}(X_{occ})$ - значение зависит от суммарной эффективной занятости $X_{occ}$ внутри $\Omega_{occ}$. Если $X_{occ}$ меньше некоторого порога, то значение штрафа достаточно велико, иначе - небольшое.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.5\paperwidth]{boxes_inv_rand.png}
\end{figure}

\begin{figure}[h]
  \centering
  \includegraphics[width=0.5\paperwidth]{inv_and_randomized_real.png}
\end{figure}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.5\paperwidth]{gradient_inv_result.png}
\end{figure}

\begin{figure}[h]
  \centering
  \includegraphics[width=0.5\paperwidth]{gradient_inv_real_result.png}
\end{figure}
...
\begin{itemize}
  \item В результате работы исследованы различные метода восстановления карты проходимости с помощью наблюдений сонаров. Оказалось, что традиционные методы восстанавливают карты с грубыми ошибками, другие подходы невозможно использовать в режиме реального времени.

  \item В работе разработаны два новых метода картирования, предложена новая прямая модель сонара.
